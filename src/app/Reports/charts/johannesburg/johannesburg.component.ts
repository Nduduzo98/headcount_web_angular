...
@Component({
  selector: 'app-johannesburg',
  templateUrl: './johannesburg.component.html',
  styleUrls: ['./johannesburg.component.scss']
})
export class JohannesburgComponent {
  inProgress:any
...
for(const dist of this.districtID){
  this.lookup.getDistrictNameByID(dist).subscribe({
    next:(names)=>{
      this.namesOfDistricts = names.districtName
      console.log(this.namesOfDistricts);
