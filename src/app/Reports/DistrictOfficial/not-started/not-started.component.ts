...
export class NotStartedComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.report.getNotStartedByDistrict(this.currentUser.districtID).subscribe({
      next:(data)=>{
  ...
  download(){
    this.report.downloadNotStarted(this.currentUser.districtID).subscribe({
  ...
  downloadExcel(){
    this.report.downloadNotStartedExcel(this.currentUser.districtID).subscribe(
      (excelBlob: Blob) => {
